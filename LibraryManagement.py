...
class Library:
    books = []
    my_book = {}
    def getAllBooks(self,text):
        crsr = conn.cursor()
        sql = f"SELECT title FROM books WHERE title = '{text}' OR author_name = '{text}' OR isbn = '{text}'"
        crsr.execute(sql)
    ...
    def getBookByAuthourName(self,author):
        crsr = conn.cursor()
        sql = f"SELECT title FROM books WHERE author_name = '{author.lower()}'"
        crsr.execute(sql)
        books = crsr.fetchall()
        for book in books:
            print("\t",book[0])
    ...
    def getBookByISBN(self,isbn):
        crsr = conn.cursor()
        sql = f"select distinct(title) from books where isbn= '{isbn}'"
        crsr.execute(sql)
        books = crsr.fetchall()
        crsr.close()
        return books
...
class InterFace(Library):
    def __init__(self):
        ...
        self.options = ["All","Author","Title","ISBN","Subject"]
        ...
        image = tk.PhotoImage(file=r"search_icon.png",width=5,height=5)
        
        self.search_button = tk.Button(top_frame ,text="search",command=lambda:self.searchBook(self.search_bar.get(),self.drop_down_menu))
    ...
    def searchBook(self,text,menu):
        option = menu.get().lower()
        search_text = text.lower()
        if option == 'author':
            Library().getBookByAuthourName(search_text)
        elif option == 'title':
            Library().getBookByName(search_text)
        elif option == 'isbn':
            Library().getBookByISBN(search_text)
        elif option == 'all':
            Library().getAllBooks(search_text)
        pass
